...
//      $Id: RooUnfoldResponse.cxx,v 1.11 2020/08/08 22:49:06 schellma Exp $
//
// Description:
//      Response Matrix
//
...
// HMS
// there is a root5/root6 difference, root5 does not have CLING so we can tell the difference that way.
          ...
          // HMS ROOT6 requires the cast which ROOT5 didn't care about
          fv= dynamic_cast<TF2*>(_func)->Integral (tru->GetXaxis()->GetBinLowEdge(jx), tru->GetXaxis()->GetBinUpEdge(jx),
          ...
          fv= dynamic_cast<TF3*>(_func)->Integral (tru->GetXaxis()->GetBinLowEdge(jx), tru->GetXaxis()->GetBinUpEdge(jx),
...
RooUnfoldResponse::Setup (const TH1* measured, const TH1* truth)
{
  ...
  if (_overflow && (_mdim > 2 || _tdim > 2)) {
    cerr << "UseOverflow setting ignored for multi-dimensional distributions" << endl;
    _overflow= 0;
  }
  SetNameTitleDefault();
  _nm= _mes->GetNbinsX() + ((_overflow) ? 2 : 0);
  _nm *= _mes->GetNbinsY() + ((_overflow && _mdim >= 2) ? 2 : 0);
  _nm *= _mes->GetNbinsZ() + ((_overflow && _mdim >= 3) ? 2 : 0);
  //_nt= _tru->GetNbinsX() * _tru->GetNbinsY() * _tru->GetNbinsZ();
  _nt= _tru->GetNbinsX() + ((_overflow) ? 2 : 0);
  _nt *= _tru->GetNbinsY() + ((_overflow && _mdim >= 2) ? 2 : 0);
  _nt *= _tru->GetNbinsZ() + ((_overflow && _mdim >= 3) ? 2 : 0);
...
RooUnfoldResponse::Setup (const TH1* measured, const TH1* truth, const TH2* response)
{
  ...
  if (_mdim == 2) _overflow= 1;
  /*cout<<"Overflow:"<<_overflow<<", _mdim:"<<_mdim<<endl;
  cout<<"(_overflow && _mdim >= 2):"<<(_overflow && _mdim >= 2)<<endl;
  cout<<"(_overflow && _mdim >= 3):"<<(_overflow && _mdim >= 3)<<endl;
  cout<<_mes<<endl;
  cout<<_mes->GetNbinsX()<<endl;
  cout<<_mes->GetNbinsY()<<endl;
  cout<<_mes->GetNbinsZ()<<endl;
  cout<<"x:"<<(_mes->GetNbinsX() + (_overflow) ? 2 : 0)<<endl;
  cout<<"y:"<<(_mes->GetNbinsY() + (_overflow && _mdim >= 2) ? 2 : 0)<<endl;
  cout<<"z:"<<(_mes->GetNbinsZ() + (_overflow && _mdim >= 3) ? 2 : 0)<<endl;*/
  int temp = 0;
  _nm= _mes->GetNbinsX();
  if (_overflow) _nm += 2;
  // y direction
  temp = _mes->GetNbinsY();
  if (_overflow && _mdim >= 2) temp += 2;
  _nm *= temp;
  // z direction
  temp = _mes->GetNbinsZ();
  if (_overflow && _mdim >= 3) temp += 2;
  _nm *= temp;
  //cout<<"_nm:"<<_nm<<endl;
  //_nt= _tru->GetNbinsX() * _tru->GetNbinsY() * _tru->GetNbinsZ();
  ...
  if (!measured || _mes->GetEntries() == 0.0) {
    ...
    for (Int_t i= 0; i<nm; i++) {
      ...
      // this +2 business doesn't make sense for 2+ dimensions
      for (Int_t j= 0; j<_nt+((_mdim==1) ? 2: 0); j++) {
  ...
  if (!truth || _tru->GetEntries() == 0.0) {
    ...
    for (Int_t j= 0; j<nt; j++) {
      ...
      // this +2 business doesn't make sense for 2+ dimensions
      for (Int_t i= 0; i<_nm+((_mdim==1) ? 2: 0); i++) {
...
RooUnfoldResponse::FindBin(const TH1* h, Double_t x, Double_t y)
{
  ...
  // this returns a global bin number in all dimensions
  
  Int_t out = h->FindFixBin(x,y)-1;
  return out;
}

// TODO this doesn't work for overflows depending on what h is. Needed for 3d unfolding
Int_t
RooUnfoldResponse::FindBin(const TH1* h, Double_t x, Double_t y, Double_t z)
{
  ...
  cout << "Warning: 3d unfolding doesn't work." << endl;
  assert(false);
...
RooUnfoldResponse::GetBinDim (const TH1* h, Int_t i)
{
  ...
  if        (ndim == 2) {
    ...
    // TODO fix 3d unfolding
    cout << "Error: 3d unfolding doesn't work." << endl;
    assert(false);
    Int_t ny= h->GetNbinsY();
