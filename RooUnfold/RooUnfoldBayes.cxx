...
//      $Id: RooUnfoldBayes.cxx,v 1.8 2021/01/11 17:48:04 ltrung Exp $
//
// Description:
//      Bayesian unfolding.
//
...
void RooUnfoldBayes::setup()
{
  ...
  // For > ROOT 5.13.0, the number of fake entries is a double (GetEffectiveEntries)
  // So I'm comparing to an arbitrarily small number.  I can't think of a better solution
  if (_res->FakeEntries() > 1e-6 ) {
  ...
  for (Int_t kiter = 0 ; kiter < _niter; kiter++) {
    ...
    if (_dosys) {
      ...
      if (kiter > 0) {
        ...
        TMatrixD dnCidPjkUpd = multiply_mt(B, _dnCidPjk); // trung
        Int_t nec= _ne*_nc;
        for (Int_t i = 0 ; i < _nc ; i++) {
...
void RooUnfoldBayes::getCovariance()
{
  ...
  if (_dosys) {
    ...
    if (_dosys!=2) {
      ...
      ABAT_MT (_dnCidPjk, Vjk, covres);
      _cov += covres;
    } else {
      _cov.ResizeTo (_nc, _nc);
      ABAT_MT (_dnCidPjk, Vjk, _cov);
    }
  }
}
