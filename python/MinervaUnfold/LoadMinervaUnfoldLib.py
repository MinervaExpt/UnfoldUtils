...
if "UNFOLDUTILSROOT" in os.environ:
  ...
  ROOT.gSystem.Load("libMinervaUnfold")

	# copy the classes from the Reflex library
	# into the "PlotUtils" namespace for more
	# straightforward access.
  for cls in CLASSES_TO_LOAD:
    setattr(MinervaUnfold, cls, getattr(ROOT.MinervaUnfold, cls))
else:
	print ( "Note: $UNFOLDUTILSROOT is not defined in the current environment.  PlotUtils libraries were not loaded.")
